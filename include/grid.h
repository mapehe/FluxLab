...
__host__ __device__ int get_flat_index(int x, int y, int t, int gridWidth,
                                       int gridHeight);
...
cuFloatComplex *allocateDeviceComplexArray(size_t x, size_t y, size_t t) {
  ...
  if (cudaStatus != cudaSuccess) {
    ...
    error_msg += std::to_string(x) + "x" + std::to_string(y) + "x" +
                 std::to_string(t) + " array): ";
...
std::vector<cuFloatComplex> getHostResults(cuFloatComplex *d_array, size_t x,
                                           size_t y, size_t t) {
  size_t total_elements = x * y * t;
  size_t mem_size = total_elements * sizeof(cuFloatComplex);
  ...
  cudaError_t cudaStatus =
      cudaMemcpy(h_array.data(), d_array, mem_size, cudaMemcpyDeviceToHost);
