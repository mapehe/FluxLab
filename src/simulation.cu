...
__global__ void simulationKernel(cuFloatComplex* d_array, int gridWidth, int gridHeight, int time) {
    ...
    const float phase = (r * spatial_freq) 
...
void run(json config) {
    ...
    const auto d_array = allocateDeviceComplexArray(params.gridWidth,
        params.gridHeight, params.iterations);
    ...
    const int progressStep = std::max(1, (int)std::round(params.iterations * 0.05));
    ...
    for (int t = 0; t < params.iterations; ++t) {
        if (t >= nextReportIteration) {
            ...
            std::cout << "[CPU] Simulation Progress: " 
                      << std::round(currentProgress / 5.0) * 5.0 << "% complete (Iteration " << t << ")\n";
                      
            nextReportIteration += progressStep; 
        }

        simulationKernel<<<numBlocks, threadsPerBlock>>>(
            d_array, 
            params.gridWidth, 
            params.gridHeight,
            t
    ...
    std::vector<cuFloatComplex> h_array = getHostResults(d_array,
        params.gridWidth, params.gridHeight, params.iterations);

    cudaFree(d_array);

    saveToBinary(params.outputFile, h_array,
        params.gridWidth, params.gridHeight, params.iterations);
}
